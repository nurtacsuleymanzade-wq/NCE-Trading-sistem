(function () {
  ...
  function smartMoneyModel(klines5, dailyRows, current, atr5) {
    ...
    var dayStart = Date.UTC(new Date().getUTCFullYear(), new Date().getUTCMonth(), new Date().getUTCDate()), prev5 = k5.filter(function(x){return x.t>=dayStart-86400000&&x.t<dayStart;}), pvBase=sum(prev5,function(x){return x.v;}), pvQuote=sum(prev5,function(x){return x.qv;});
    if (pvBase > 0 && pvQuote > 0) levels.push({kind:'PVWAP',price:pvQuote/pvBase,weight:1.12,provenance:'DERIVED · previous UTC day quote/base volume'});
